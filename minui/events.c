...
//            fprintf(stderr,"/dev/input/%s\n", de->d_name);
    ...
    if(pipe(kick_pipe_fds))
        perror("unable to create kick pipe");
    ev_fds[ev_count].fd = kick_pipe_fds[0];
    ev_fds[ev_count].events = POLLIN;
    ev_fdinfo[ev_count].cb = input_cb;
    ev_fdinfo[ev_count].data = data;
    ev_count++;
...
void ev_kick(void)
{
    ...
    ev.type = EV_SYN;
    /* send a SYN (==null) event to kick the event loop */
    write(kick_pipe_fds[1], &ev, sizeof(ev));
}
