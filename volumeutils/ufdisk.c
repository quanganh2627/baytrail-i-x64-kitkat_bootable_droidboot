...
static struct block_dev *open_disk(char *disk)
{
	...
	if (r < 0 || sec_size != 512)
	{
		fprintf(stderr, "sec_size != 512 ? what kind of disk is this?");
		return NULL;
	}
	r = ioctl(fd,BLKGETSIZE, &num_sec);
	if (r < 0)
	{
		fprintf(stderr, "unable to get disk size");
		return NULL;
	}
	...
	if (r < 0)
	{
		fprintf(stderr, "unable to get disk geometry");
		return NULL;
	}
...
static int write_partitions(struct block_dev *dev)
{
	...
	r = write_mbr(dev);
	if (r < 0) {
		fprintf(stderr, "write_mbr error!\n");
		return r;
	}
	...
	if (r < 0)
		fprintf(stderr, "unable to load partition table");
	return r;
}
...
int ufdisk_create_partition(void)
{
	...
	if (max_space < allocated_space) {
		fprintf(stderr, "emmc is too small for this partition table! %dM VS %dM\n",
	...
	for (i = 0; i < num_volumes; ++i) {
		...
		if (wait_device_creation_timeout(v->device, DEVICE_CREATION_TIMEOUT) != 0) {
			fprintf(stderr, "fatal: unable to create partition: %s\n", v->device);
			return -ENODEV;
		}
	}
